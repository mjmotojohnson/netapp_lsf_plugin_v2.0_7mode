 ...
 */
#include "sysntap.h"
#include "tools/expat-2.0.1/lib/expat_external.h"
#include "tools/expat-2.0.1/lib/expat.h"
...
compare_thresholds(char *filername, char *volname) {
    ...
    if ((vpo != NULL) && (vpo->policy.maxdiskbusy != 0)) {
        ...
        if ((policy.maxdiskbusy !=0) && (v->maxdiskbusy > policy.maxdiskbusy))
	    return(1);
    }
    ...
    if ((vpo != NULL) && (vpo->policy.maxavgvollat != 0)) {
        ...
        if ((policy.maxavgvollat != 0) && (v->avglatency > policy.maxavgvollat))
	    return(1);
	...
	if (v->availinodes < vpo->policy.mininodes)
        ...
        if ((policy.mininodes != 0) && (v->availinodes < policy.mininodes))
	    return(1);
    }
	...
	if (asize < vpo->policy.minsize)
	   ...
	   return(1);
        ...
        if ((policy.minsize != 0) && (asize < policy.minsize))
	   return(1);
    }
...
init(void)
{
    ...
    /* Initialize the global policy to zero */
    memset(&policy, 0, sizeof(struct loadpolicy));
...
/*
 * delSpace() - deletes the spaces or tabs within a string
 */
static void
delSpace(char *str) 
{
...
%s: failed allocate: %s",
	...
	log_msg(MSG_DEBUG, "%s: Installed volume record %s %0.5f", fname, wbuf, 
vtmp->avglatency);
	return(0);
    }
